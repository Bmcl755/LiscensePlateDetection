...
#_______Extension-Helper-Functions________
def separatePlateFromImg(pixel_array, bbox_min_y, bbox_min_x, bbox_max_y, bbox_max_x):
    result = createInitializedGreyscalePixelArray(bbox_max_x-bbox_min_x, bbox_max_y-bbox_min_y)
    for height in range(len(result)):
        for width in range(len(result[height])):
            result[height][width] = pixel_array[bbox_min_y+height][bbox_min_x+width]
    return result
...
def main():
    ...
    #Extension area
    numPlateImageArray = separatePlateFromImg(px_array, bbox_min_y, bbox_min_x, bbox_max_y, bbox_max_x)
    axs1[1, 1].set_title('Image of detected plate')
    axs1[2, 1].imshow(numPlateImageArray, cmap='gray')
    axs1[2, 0].set_visible(False)
    #Save plate image
    extent = axs1[2, 1].get_window_extent().transformed(fig1.dpi_scale_trans.inverted())
    pyplot.savefig("output_images/numPlateImage.png", bbox_inches=extent)
